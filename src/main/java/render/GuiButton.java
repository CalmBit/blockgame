...
public class GuiButton extends GuiBase {
    ...
    public void render(Matrix4f proj) {
        ...
        GL31.glBindVertexArray(GuiRenderer.gvao);
        GL31.glBindBuffer(GL31.GL_ARRAY_BUFFER, GuiRenderer.gvbo);

        try {
            _stack = MemoryStack.stackPush();
            GL31.glUniformMatrix4fv(GuiRenderer.guiProj, false, proj.get(_stack.mallocFloat(16)));
        } finally {
            _stack.pop();
        }
