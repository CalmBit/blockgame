...
public class Camera {
    private Vector3f _pos = new Vector3f(0.0f, 84.0f, 0.0f);
    ...
    public void updatePosition(boolean[] keyStates) {
        if (keyStates[GLFW.GLFW_KEY_W])
            _pos.add(new Vector3f(_front.x * 0.5f, _front.y * 0.5f, _front.z * 0.5f));
        if (keyStates[GLFW.GLFW_KEY_S])
            _pos.sub(new Vector3f(_front.x * 0.5f, _front.y * 0.5f, _front.z * 0.5f));
        if (keyStates[GLFW.GLFW_KEY_A])
            _pos.sub(new Vector3f(_front.x * 0.5f, _front.y * 0.5f, _front.z * 0.5f).cross(_up));
        if (keyStates[GLFW.GLFW_KEY_D])
            _pos.add(new Vector3f(_front.x * 0.5f, _front.y * 0.5f, _front.z * 0.5f).cross(_up));
    }
